...
public class EmployeeUpdate extends Card {
  ...
  private EmpDatabase empData = new EmpDatabase(
  ...
  private void atLeft() {
    ...
    try (ResultSet data = empData.getData(this.id)) {
      while (data.next()) {
        ...
        if (text.equalsIgnoreCase("male")) male.setSelected(
          true
  ...
  protected boolean isEmpty() {
    ...
    if (
      ...
      phone.getText().trim().equalsIgnoreCase("n/a")
    ) {
      errors.append("- Please enter contact number.\n");
    }
    if (
      email.getText().trim().isEmpty() ||
      email.getText().trim().equalsIgnoreCase("n/a")
    ) {
